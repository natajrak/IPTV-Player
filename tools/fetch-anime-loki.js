 ...
 *                     ใช้เมื่อ source เรียงตอนครบตั้งแต่ตอนแรก — เลขตอนถูก map
 *                     แบบเรียงลำดับแทนการอ่านเลขจากชื่อตอนฝั่งเว็บ
 *                     ห้ามใช้ร่วมกับ --part (list ที่ถูก filter จะเริ่มนับ 1 ใหม่)
...
let { seriesUrl, tmdbKey, customOutput, mainSlugArg, idPrefixArg,
      ...
      forceTmdbId, tmdbSeasonNum, epOffset, splitEps, autoSplit, partNum,
...
async function main() {
  ...
  try {
    ...
    // --split-eps: ใช้ index-based map เฉพาะเมื่อส่ง flag (เลขตอนปกติของ loki
    // parse จากชื่อฝั่งเว็บ + มี --part filter จึงห้ามเปลี่ยน default behavior)
    ...
    if (ms && episodes.some((e) => e.isPending)) {
      throw new Error('multi-season: มีตอน "(รออัพ)" ที่จะถูกข้าม ทำให้แบ่ง season คลาดเคลื่อน — รอให้ครบก่อน');
    }
    ...
    for (let i = 0; i < episodes.length; i++) {
      ...
      if (ms) {
        ...
        tmdbEp = tmdbEpisodes.find((e) => e.episode_number === tmdbEpNum)
